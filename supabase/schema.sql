...
-- wiped and never replaced. Same pattern as record_dose/edit_dose_log
-- below: security invoker (the default), so the "own medications"/"own
-- schedule times" RLS policies still apply inside it, and `for update`
-- locks the row for the duration.
--
